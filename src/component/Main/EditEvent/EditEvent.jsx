...
import { useGetSingleEventQuery, useUpdateEventMutation } from "../../../redux/features/events/eventsApi";
...
const EditEvent = () => {
  ...
  const [description, setDescription] = useState(""); // Manage description separately for Jodit

  const { data: eventData, isLoading: isFetching } = useGetSingleEventQuery(id, { skip: !id });
  ...
  useEffect(() => {
    if (eventData?.data?.attributes) {
      ...
      const parsedEventDate = event?.eventDate ? dayjs(event.eventDate) : null;
      setDescription(event.description || ""); // Set Jodit initial value
      ...
      form.setFieldsValue({
        ...
        eventDate: parsedEventDate && parsedEventDate.isValid() ? parsedEventDate : null,
      });

      if (event.image?.imageUrl) {
        setImagePreview(`${imageBaseUrl}${event.image.imageUrl}`);
      }
    }
  }, [eventData, form]);
  ...
  const onFinish = async (values) => {
    try {
      ...
      formData.append("description", description); // use state description (from Jodit)
      ...
      message.error(`Failed to update event: ${error.message || "Unknown error"}`);
    }
  };
        ...
        >
          <JoditEditor
            ...
            onBlur={(newContent) => {
              ...
              form.setFieldsValue({ description: newContent }); // Update form value
            }}
          />
        </Form.Item>
