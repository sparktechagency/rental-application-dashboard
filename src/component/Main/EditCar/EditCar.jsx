...
const EditCarForm = () => {
  ...
  const { data: car, isLoading: isCarLoading, error: carError } = useGetSingleCarQuery(id);
  ...
  // Pre-populate form with car data
  useEffect(() => {
    if (car) {
      form.setFieldsValue({
        make: car.make,
      ...
      if (car.image) {
        setImages([{
          id: Date.now(),
          url: car.image,
          file: null, // Existing image, no file needed
        }]);
      }
    }
  }, [car, form]);
  ...
  const handleUpdate = async (values) => {
    ...
    try {
      ...
      const data = JSON.stringify({ ...values, color, status: car.status });
      newFormData.append("data", data);
      images.forEach((image) => {
        if (image.file) { // Only append new images
          newFormData.append("image", image.file);
        }
      });
      const response = await updateCar({ id, data: newFormData }).unwrap();
  ...
  if (carError) {
    return <div>Error loading car data: {carError?.data?.message || "Something went wrong"}</div>;
  }
  ...
  console.log("Card Data:", car);

  return (
    <div className="w-full px-5 space-y-5 bg-[#F5F5F5]">
      <Form form={form} onFinish={handleUpdate} layout="vertical" className="w-full space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
          ...
          <Form.Item
            name="camera"
            rules={[{ required: true, message: "Please enter the car camera" }]}
          >
          ...
          <Form.Item
            name="bluetooth"
            rules={[{ required: true, message: "Please enter the car bluetooth" }]}
          >
          ...
          <Form.Item
            name="seats"
            rules={[{ required: true, message: "Please enter the car seats" }]}
          >
